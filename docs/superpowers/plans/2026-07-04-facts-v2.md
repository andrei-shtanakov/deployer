...
Post-review backlog: normalize-regex drift-guard test (facts vs hints); quoted-env-value bypass in install_strategy; path-prefixed/sh -c invocations bypass (degrade safely to L2); CMD-without-arg system-prompt nudge (decide deliberately — changes the measured subject; note in run-comparability record if adopted).
