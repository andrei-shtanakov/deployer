# Provenance: `record-format` (admission case, A §8.3 (1) step 1, unknown `format_version` in the record)
...
`record.json` rewritten in place with `format_version` `"2"` (not re-signed, directory not renamed).
