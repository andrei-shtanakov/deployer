# Provenance: `snapshot-format` (admission case, A §8.3 (1) step 1, unknown `format_version` in the snapshot)
...
`snapshot.json` rewritten in place with `format_version` `"2"` (record unchanged).
