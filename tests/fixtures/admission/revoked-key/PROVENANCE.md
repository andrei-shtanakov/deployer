# Provenance: `revoked-key` (admission case, A §8.3 (1) step 3, revoked key)
...
the set is unchanged; the case's own `trust/` keeps the test key in `allowed_signers` and lists it in `revoked_keys`, so only `-r` refuses it.

Trust store for the replay: `{"kind": "dir", "path": "revoked-key/trust"}` (see `expected.json`).
...
- `trust/allowed_signers`: added
- `trust/revoked_keys`: added
