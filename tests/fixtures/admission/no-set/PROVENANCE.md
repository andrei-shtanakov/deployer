# Provenance: `no-set` (admission case, A §8.3 (1) step 1, no set)
...
`.deployer/authoring/Dockerfile.current` removed (the set directory stays: without the pointer no set is published).
