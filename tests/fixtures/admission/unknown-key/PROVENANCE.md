# Provenance: `unknown-key` (admission case, A §8.3 (1) step 3, unknown key)
...
`record.json.sig` re-made with `unknown-test-key`, which no trust store lists (record and directory unchanged).
