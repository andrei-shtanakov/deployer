# Provenance: `trust-inside` (admission case, A §8.3 (1) step 0, trust dir inside the repo)
...
`tree/ci-trust/allowed_signers` added, a byte copy of the shared `trust/allowed_signers`; the trust dir is pointed at it inside the restored tree.

Trust store for the replay: `{"kind": "inside_tree", "path": "ci-trust"}` (see `expected.json`).
