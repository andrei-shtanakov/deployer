# Provenance: `bad-signature` (admission case, A §8.3 (1) step 3, well-formed but invalid signature)
...
`record.json.sig` replaced by a well-formed signature of the test key over other bytes (`other bytes\n`).
