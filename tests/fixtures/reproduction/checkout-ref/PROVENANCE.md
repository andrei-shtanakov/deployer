# Provenance: `checkout-ref` (negative case, spec §8.A)
...
- `tree/.github/workflows/diagnosis-polygon.yml`: `with: {ref: main}` on the checkout step; `tree-listing.json` entry sha recomputed.
