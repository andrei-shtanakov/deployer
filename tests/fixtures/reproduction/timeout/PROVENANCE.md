# Provenance: `timeout` (negative case, spec §8.A)
...
- `local.timeout` added, containing `true`: the harness's fake `podman build` raises `subprocess.TimeoutExpired`. `local.*` kept but not replayed.
