...
The shipped package lives under `src/deployer/`: `facts` (project scan), `models` (`DeployTarget`, reports), `hints` (curated apt table), `llm` + `author` (prompt and the authoring loop), `artifacts` (sentinel parse/render), `verify` (L1/L2), `runtime` (`ContainerRuntime`, the single container-subprocess chokepoint), `reproduce` (`diagnose --reproduce`: tree/workflow restoration at the run's checkout SHA, a closed list of offline checks, a local rebuild through its own adapter, CI-vs-local comparison — findings with status and evidence, no causal class), `provenance` (`author --signing-key`: the signed authoring set under `.deployer/authoring/`, `ssh-keygen -Y` signing, the out-of-repo trust store behind `deployer trust`), `admission` (the `admission` section of `diagnose --reproduce`: ownership, a defect from the closed two-class catalogue, the CI-vs-local link by recorded template rows → `admitted` | `insufficient_grounds`; `accept_for_fix`, the consumer gate), `fix` (`deployer fix`/`fix publish`/`fix confirm`: authors the one instruction that removes an `admitted` defect, proves it locally and confirms its removal in CI — `fix.json` schema 1.0, statuses `locally_confirmed` | `fix_proposed` | `ci_confirmed` | `stopped`; both proof stages read recording-backed template rows — local Podman and CI BuildKit, all enabled; end-to-end acceptance evidence under `tests/fixtures/e2e/`), `bench`, `cli`. Tests live under `tests/`; container- and LLM-dependent checks are marker-gated. The corpus (12 synthetic cases, up from 11 with the `atp-agent` ATP-smoke case) and the committed golden baseline live under `corpus/`; raw bench runs land in gitignored `.deployer-runs/`.
